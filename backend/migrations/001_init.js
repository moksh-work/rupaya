...
exports.up = async function(knex) {
  ...
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_users_created_at ON users(created_at)");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_users_status ON users(account_status)");

  // Devices Table
  const hasDevicesTable = await knex.schema.hasTable('devices');
  if (!hasDevicesTable) {
  ...
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_devices_user_id ON devices(user_id)");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_devices_fingerprint ON devices(device_fingerprint)");

  // Accounts Table
  const hasAccountsTable = await knex.schema.hasTable('accounts');
  if (!hasAccountsTable) {
  ...
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_accounts_user_id ON accounts(user_id)");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_accounts_type ON accounts(account_type)");
  ...
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_categories_user_id ON categories(user_id)");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_categories_type ON categories(category_type)");
