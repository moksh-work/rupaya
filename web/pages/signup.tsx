    ...
    <>
      ...
      <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #3EC6E0 0%, #1A4D7A 100%)' }}>
        <div style={{ background: 'white', borderRadius: 24, padding: 32, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', maxWidth: 400, width: '90%' }}>
          <h1 style={{ textAlign: 'center', color: '#1A4D7A', fontWeight: 700, fontSize: 28, margin: 0 }}>Sign Up</h1>
          <form onSubmit={handleSubmit} style={{ marginTop: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
            <input type="text" placeholder="Name" required value={name} onChange={e => setName(e.target.value)} style={{ padding: 12, borderRadius: 8, border: '1px solid #ccc' }} />
            ...
            <button type="submit" disabled={loading} style={{ background: '#1A4D7A', color: 'white', borderRadius: 8, padding: '12px 0', fontWeight: 600, fontSize: 16, border: 'none' }}>{loading ? 'Signing up...' : 'Sign Up'}</button>
          </form>
          {error && <div style={{ color: 'red', marginTop: 12, textAlign: 'center' }}>{error}</div>}
          <div style={{ marginTop: 16, textAlign: 'center' }}>
            <span>Already have an account? </span>
            <Link href="/login" style={{ color: '#3EC6E0', fontWeight: 500 }}>Login</Link>
          </div>
        </div>
      </main>
    </>
